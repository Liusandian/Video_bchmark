...
class CameraPredict:
    ...
    def detect_bullet_time_motion(self, tracks, visibility, h, w, face_poses=None):
        """
        通用子弹时间运镜检测（支持人像、建筑、风景等各种场景）
        主要基于CoTracker特征点的环形运动轨迹分析
        tracks: T N 2 (numpy, 像素坐标)
        visibility: T N (numpy, [0-1])
        face_poses: [(yaw, pitch, roll), ...] 可选的人脸姿态角度序列
        返回: (bullet_time_type, confidence, details)
        """
        if tracks.shape[0] < 8:  # 子弹时间需要足够的帧数，降低要求以适应更多场景
            return "no_bullet_time", 0.0, {}
        
        cx, cy = w * 0.5, h * 0.5
        
        # === 1. 基于CoTracker特征点的环形运动分析 ===
        circular_scores = []
        radial_movements = []  # 径向运动分析
        angular_velocities = []  # 角速度分析
        ...
        for t in range(tracks.shape[0] - 1):
            ...
            if len(angle_diffs) > 0:
                ...
                angular_velocities.append(angular_velocity_std)
            ...
            radial_change = np.mean(distances_next - distances_curr)
            radial_movements.append(radial_change)
        ...
        radial_movements = np.array(radial_movements)
        angular_velocities = np.array(angular_velocities)
        
        # 计算环形运动的主要指标
        mean_circular_motion = np.mean(circular_scores)
        circular_direction_consistency = self._calculate_direction_consistency(circular_scores)
        
        # 计算径向运动稳定性（子弹时间中径向变化应该较小）
        radial_stability = 1.0 / (1.0 + np.std(radial_movements))
        
        # 计算角速度一致性（子弹时间中角速度应该相对恒定）
        angular_consistency = 1.0 / (1.0 + np.mean(angular_velocities))
        
        # 总角度变化量
        total_rotation_rad = abs(mean_circular_motion) * len(circular_scores)
        total_rotation_deg = total_rotation_rad * 180 / np.pi
        
        # === 3. 多尺度环形运动检测 ===
        # 分析不同区域的特征点运动一致性
        region_consistency = self._analyze_regional_motion_consistency(tracks, visibility, cx, cy, h, w)
        
        # 检测运动的周期性（子弹时间通常有明显的周期性）
        motion_periodicity = self._detect_motion_periodicity(circular_scores)
        
        # === 4. 人脸姿态辅助分析（如果可用） ===
        face_angle_support = 0.0
        face_angle_range = 0.0
        face_detection_ratio = 0.0
        
        if face_poses is not None and len(face_poses) > 0:
            valid_poses = [pose for pose in face_poses if pose is not None]
            if len(valid_poses) >= 3:
                yaw_angles = [pose[0] for pose in valid_poses]
                face_angle_range = max(yaw_angles) - min(yaw_angles)
                face_detection_ratio = len(valid_poses) / len(face_poses)
                
                # 人脸角度变化与特征点运动的一致性
                if abs(face_angle_range) > 30:  # 有明显的人脸角度变化
                    face_angle_support = min(1.0, abs(face_angle_range) / 180.0)
        
        # === 5. 综合分析和结果判断 ===
        details = {
            'mean_circular_motion': float(mean_circular_motion),
            'circular_direction_consistency': float(circular_direction_consistency),
            'radial_stability': float(radial_stability),
            'angular_consistency': float(angular_consistency),
            'total_rotation_deg': float(total_rotation_deg),
            'region_consistency': float(region_consistency),
            'motion_periodicity': float(motion_periodicity),
            'face_angle_range': float(face_angle_range),
            'face_detection_ratio': float(face_detection_ratio),
            'face_angle_support': float(face_angle_support),
            'num_motion_frames': len(circular_scores),
            'motion_frame_ratio': len(circular_scores) / tracks.shape[0]
        }
        
        # 计算综合置信度
        # 主要权重：环形运动一致性(40%) + 径向稳定性(25%) + 角度一致性(20%) + 区域一致性(15%)
        motion_confidence = (
            circular_direction_consistency * 0.40 +
            radial_stability * 0.25 +
            angular_consistency * 0.20 +
            region_consistency * 0.15
        )
        ...
        # 周期性运动加成
        motion_confidence = motion_confidence * (0.85 + motion_periodicity * 0.15)
        
        # 角度变化阈值（适应不同场景）
        angle_90 = 75   # 90° 子弹时间的最小角度要求
        angle_180 = 150 # 180° 子弹时间的最小角度要求
        angle_270 = 240 # 270° 子弹时间的最小角度要求
        angle_360 = 320 # 360° 子弹时间的最小角度要求
        
        # 综合置信度阈值检查
        confidence_threshold = 0.6  # 降低阈值以适应更多场景
        if motion_confidence < confidence_threshold:
            return "no_bullet_time", float(motion_confidence), details
        
        # 判断子弹时间类型
        if total_rotation_deg >= angle_90:
            if total_rotation_deg >= angle_360:
                bullet_type = "bullet_time_360"
            elif total_rotation_deg >= angle_270:
                bullet_type = "bullet_time_270"
            elif total_rotation_deg >= angle_180:
                bullet_type = "bullet_time_180"
            else:
                bullet_type = "bullet_time_90"
            
            return bullet_type, float(motion_confidence), details
        else:
            return "no_bullet_time", float(motion_confidence), details
    
    def _calculate_direction_consistency(self, circular_scores):
        """计算环形运动方向的一致性"""
        if len(circular_scores) < 2:
            return 0.0
        ...
        # 计算运动幅度的稳定性
        motion_stability = 1.0 / (1.0 + np.std(np.abs(circular_scores)))
        
        return dominant_ratio * motion_stability
    
    def _analyze_regional_motion_consistency(self, tracks, visibility, cx, cy, h, w):
        """分析不同区域特征点运动的一致性"""
        # 将画面分为内、中、外三个环形区域
        scale = min(h, w)
        inner_radius = scale * 0.2
        outer_radius = scale * 0.4
        
        regional_consistencies = []
        ...
        for t in range(tracks.shape[0] - 1):
            ...
            # 计算点到中心的距离
            distances = np.linalg.norm(valid_curr - np.array([cx, cy]), axis=1)
            
            # 分区域分析
            for min_radius, max_radius in [(0, inner_radius), (inner_radius, outer_radius), (outer_radius, scale)]:
                region_mask = (distances >= min_radius) & (distances < max_radius)
                
                if region_mask.sum() >= 3:  # 该区域有足够的点
                    region_curr = valid_curr[region_mask]
                    region_next = valid_next[region_mask]
                    
                    # 计算该区域的角度变化
                    angles_curr = np.arctan2(region_curr[:, 1] - cy, region_curr[:, 0] - cx)
                    angles_next = np.arctan2(region_next[:, 1] - cy, region_next[:, 0] - cx)
                    ...
                    # 计算该区域运动的一致性
                    if len(angle_diffs) > 0:
                        region_consistency = 1.0 / (1.0 + np.std(angle_diffs))
                        regional_consistencies.append(region_consistency)
        
        return np.mean(regional_consistencies) if regional_consistencies else 0.0
    
    def _detect_motion_periodicity(self, circular_scores):
        """检测运动的周期性特征"""
        if len(circular_scores) < 6:
            return 0.0
        
        try:
            # 计算自相关函数来检测周期性
            scores = np.array(circular_scores)
            correlations = []
            
            # 检测可能的周期长度
            for lag in range(1, min(len(scores)//2, 10)):
                if len(scores) > lag:
                    correlation = np.corrcoef(scores[:-lag], scores[lag:])[0, 1]
    ...
    def predict_with_segmentation(self, video, fps=16, end_frame=-1):
        ...
        # 5) 子弹时间检测（通用方法，人脸姿态为可选参数）
        bullet_type, bullet_conf, bullet_details = self.detect_bullet_time_motion(tracks, vis, h, w, face_poses)
    ...
    def predict_universal_bullet_time(self, video, fps=16, end_frame=-1):
        """
        通用子弹时间检测（不依赖人脸检测，适用于所有场景）
        专门针对建筑、风景、物体等非人像场景的子弹时间检测
        """
        # 1) 获取轨迹与可见性
        tracks, vis = self.infer_with_visibility(video, fps=fps, end_frame=end_frame)
        h, w = video.shape[3], video.shape[4]
        
        # 2) 通用子弹时间检测（不使用人脸信息）
        bullet_type, bullet_conf, bullet_details = self.detect_bullet_time_motion(tracks, vis, h, w, face_poses=None)
        ...
        # 4) 增强标准运动结果
        enhanced_standard = standard.copy()
        if bullet_type != "no_bullet_time" and bullet_conf > 0.6:
            if bullet_type not in enhanced_standard:
                enhanced_standard.append(bullet_type)
