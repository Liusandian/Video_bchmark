...
# 导入子弹时间检测模块
try:
    from .bullet_time import VGGTHeadPoseEstimator
    _HAS_BULLET_TIME = True
except ImportError:
    _HAS_BULLET_TIME = False
    print("警告: 子弹时间检测模块未找到")
...
class CameraPredict:
    def __init__(self, device, submodules_list):
        ...
        # 初始化VGGT头部姿态估计器（用于子弹时间检测）
        self.pose_estimator = None
        if _HAS_BULLET_TIME and 'vggt_path' in submodules_list:
            try:
                vggt_path = submodules_list.get('vggt_path')
                vggt_weights = submodules_list.get('vggt_weights', None)
                self.pose_estimator = VGGTHeadPoseEstimator(device, vggt_path, vggt_weights)
                print("✓ VGGT头部姿态估计器初始化成功")
    ...
    def detect_bullet_time_by_pose(self, video_path, yaw_threshold=75.0):
        """基于头部姿态的子弹时间检测"""
        ...
        if len(valid_poses) < 10:
            return False, 0.0, {'error': 'insufficient_valid_poses'}
        
        # 分析yaw角度变化
        yaw_angles = np.array([pose[0] for pose in valid_poses])
        ...
        # 判断是否为子弹时间
        is_bullet_time = total_rotation >= yaw_threshold and consistency >= 0.7
        confidence = min(1.0, (total_rotation / 180.0) * consistency)
        
        details = {
            'total_yaw_rotation': total_rotation,
            'direction_consistency': consistency,
            'valid_frames': len(valid_poses),
            'total_frames': len(poses)
        }
        
        return is_bullet_time, confidence, details
    
    def predict_with_bullet_time(self, video_path, fps, end_frame):
        """增强的预测方法，包含子弹时间检测"""
        # 读取视频数据
        ...
        # 子弹时间检测
        bullet_time_detected, bullet_confidence, bullet_details = self.detect_bullet_time_by_pose(video_path)
        
        # 检查是否有环绕运镜
        has_orbit = "orbits" in standard_results
        
        # 综合判断子弹时间
        final_bullet_time = bullet_time_detected and (has_orbit or bullet_confidence > 0.8)
        
        # 构建增强结果
        enhanced_results = standard_results.copy()
        if final_bullet_time:
        ...
        return {
            ...
            'bullet_time': {
                'detected': final_bullet_time,
                'confidence': bullet_confidence,
                'details': bullet_details
            },
            'has_orbit_motion': has_orbit
        }
...
def compute_camera_motion_with_bullet_time(json_dir, device, submodules_dict, **kwargs):
    """增强的相机运动评估，包含子弹时间检测"""
    camera = CameraPredict(device, submodules_dict)
    _, prompt_dict_ls = load_dimension_info(json_dir, dimension='camera_motion', lang='en')
    ...
    for prompt_dict in tqdm(prompt_dict_ls):
        ...
        for video_path in video_paths:
            ...
            # 使用增强的预测方法
            enhanced_result = camera.predict_with_bullet_time(video_path, fps, end_frame)
            
            # 标准运镜评分
            standard_motions = enhanced_result['standard_motions']
            video_score = 1.0 if label in standard_motions else 0.0
            
            # 子弹时间特殊处理
            if label == 'bullet_time' or 'bullet_time' in label.lower():
                bullet_detected = enhanced_result['bullet_time']['detected']
            ...
            video_results.append({
                ...
                'bullet_time_details': enhanced_result['bullet_time']
            })
            sim.append(video_score)
