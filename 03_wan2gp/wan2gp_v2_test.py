...
def _parse_args():
    ...
    parser.add_argument("--low_memory", action="store_true", help="Enable low memory mode for CPU")
...
def merge_videos(video1_path, transition_video_path, video2_path, output_path):
    ...
    # 修复: 使用单引号包围f-string，内部使用双引号
    list_file.write(f'file "{video1_abs}"\n'.encode())
    list_file.write(f'file "{transition_abs}"\n'.encode())
    list_file.write(f'file "{video2_abs}"\n'.encode())
    list_file.close()
...
if not torch.cuda.is_available():
    print("="*80)
    print("警告: 在CPU上运行Wan2GP将非常缓慢!")
    print("一个短视频可能需要数小时甚至数天才能完成")
    print("建议使用配备NVIDIA GPU的系统或云服务")
    print("="*80)
